var QuickClick = {	
	...
	deviceIsIOS4: this.deviceIsIOS && (/OS 4_\d(_\d)?/).test(navigator.userAgent),
	// iOS 6.0(+?) requires the target element to be manually derived
	deviceIsIOSWithBadTarget: this.deviceIsIOS && (/OS ([6-9]|\d{2})_\d/).test(navigator.userAgent),
	...
	// Coordinates of the touchStart event
	touchStartX: 0,
	touchStartY: 0,
	...
	textFields: ['email', 'number', 'password', 'search', 'tel', 'text', 'url'],
	
	// The constructor function. Pass in a DOM layer and, if necessary, and options object
	init: function(layer, options) {
		...
		if (options) {
			this.touchBoundary = options.touchBoundary ? options.touchBoundary : 10;
			this.tapDelay = options.tapDelay ? options.tapDelay : 200;
		}
		...
		layer.addEventListener('touchcancel', this.onTouchCancel.bind(this), false);
	...
	needsClick: function(target) {
		...
		if ((/\bselect\-target\b/).test(target.className)) {
			console.log(true);
			return true;
		}
	 ...
	 * @returns {boolean}
	 */
	isTextField: function(targetElement) {
		return (
			targetElement.tagName.toLowerCase() === 'textarea' ||
			this.textFields.indexOf(targetElement.type) !== -1
		);
	},
	...
	onTouchEnd: function(event) {
		...
		// Fixes duplicate tabs opening on external links (iOS)
		if (targetElement.parentNode.getAttribute('target') === '_blank') {
			return true;
		}
...
if (typeof define !== 'undefined' && define.amd) {
	...
	define(function() { return QuickClick; });
